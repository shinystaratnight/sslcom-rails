        ...
        scope :search_with_csr, lambda { |term, _options = {}|
          ...
          if term.present?
            ...
            sql = %(MATCH (csrs.common_name, csrs.body, csrs.decoded) AGAINST ('#{term}') OR
                    MATCH (signed_certificates.common_name, signed_certificates.url, signed_certificates.body, signed_certificates.decoded, signed_certificates.ext_customer_ref, signed_certificates.ejbca_username) AGAINST ('#{term}') OR
                    ...
                    MATCH (contacts.first_name, contacts.last_name, contacts.company_name, contacts.department, contacts.po_box, contacts.address1,
                    contacts.address2, contacts.address3, contacts.city, contacts.state, contacts.country, contacts.postal_code, contacts.email,
                    contacts.notes, contacts.assumed_name, contacts.duns_number) AGAINST ('#{term}') OR
                    MATCH (users.login, users.email) AGAINST ('#{term}')).squish
            result = result.joins{ csrs.outer }
                           .joins{ csrs.outer.signed_certificates.outer }
                           .joins{ ssl_account.outer }
                           .joins{ ssl_account.users.outer }
                           .joins{ registrants.outer }
                           .where(sql)
          end
          ...
          cc_query = (cc_query || ::CertificateContent).joins{ csrs } unless
              (keys & %i[country strength common_name organization organization_unit state subject_alternative_names locality decoded]).empty?
          cc_query = (cc_query || ::CertificateContent).includes(csr: :signed_certificates) unless
          ...
          %w(postal_code signature fingerprint).each do |field|
            query = filters[field.to_sym]
            cc_query = cc_query.where{(csr.signed_certificates.send(field.to_sym) =~ "%#{query}%")} if query
          end
          ...
          %w(country strength).each do |field|
            query = filters[field.to_sym]
            cc_query = cc_query.where{(csr.signed_certificates.send(field.to_sym) >> query.split(',')) | (csrs.send(field.to_sym) >> query.split(','))} if query
          end
          ...
          %w(common_name organization organization_unit state subject_alternative_names locality decoded).each do |field|
            query = filters[field.to_sym]
            cc_query = cc_query.where{(csr.signed_certificates.send(field.to_sym) =~ "%#{query}%") | (csrs.send(field.to_sym) =~ "%#{query}%") } if query
          end
          %w(address).each do |field|
            query = filters[field.to_sym]
            cc_query = cc_query.where{(csr.signed_certificates.address1 =~ "%#{query}%") | (csr.signed_certificates.address2 =~ "%#{query}%")} if query
          end
          %w(login email).each do |field|
            query = filters[field.to_sym]
            result = result.where{(ssl_account.users.send(field.to_sym) =~ "%#{query}%")} if query
          end
          ...
          %w(cc_tags).each do |field|
            ...
            cc_results = (@result_prior_co_tags || result).joins(certificate_contents: [:tags]).where(tags: { name: query.split(',') })
