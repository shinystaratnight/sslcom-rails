  ...
  def self.apply_for_certificate(certificate_order, options={})
    ...
    if options[:mapping].profile_name=~/EV/ and (approval_res.try(:body).blank? or approval_res.try(:body)=="[]" or
      ...
      options.merge!(
          ...
          username: cc.csr.sslcom_usernames.compact.first
      ) if options[:mapping].profile_name=~/EV/
    end

    req, res = call_ca(host, options, issue_cert_json(options))
    api_log_entry = cc.csr.sslcom_ca_requests.create(
        request_url: host,
        parameters: req.body,
        method: "post",
        response: res.try(:body),
        ca: options[:ca_name] || ca_name(options)
    )
