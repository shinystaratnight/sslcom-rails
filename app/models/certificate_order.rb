...
class CertificateOrder < ApplicationRecord
  ...
  has_many    :cdns
  has_many    :certificate_contents, dependent: :destroy, after_add: proc { |p, _d| p.certificate_content(true) }
  ...
  default_scope do
    where{ (workflow_state << %w[canceled refunded charged_back]) & (is_expired != true) }
  ...
  scope :search_with_csr, lambda { |term, _options = {}|
    term ||= ''
    ...
    filters.each do |fn, _fv|
      term.delete_if { |s| s =~ Regexp.new(fn.to_s + "\\:\\'?([^']*)\\'?"); filters[fn] ||= Regexp.last_match(1); Regexp.last_match(1) }
    end
    ...
    %w[status].each do |field|
      query = filters[field.to_sym]
      cc_query = cc_query.where{ workflow_state >> query.split(',') } if query
    end
    ...
    %w[address].each do |field|
      ...
      cc_query = cc_query.where do
        (csr.signed_certificates.address1 =~ "%#{query}%") |
          (csr.signed_certificates.address2 =~ "%#{query}%")
      end
    end
    ...
    %w[account_number].each do |field|
      ...
      result = result.where do
        (ssl_account.send(field.to_sym) =~ "%#{query}%")
      end
    end
  ...
  scope :pending, lambda {
                    not_test.joins(:certificate_contents).where do
                      certificate_contents.workflow_state >>
                        %w[pending_validation validated]
                    end .order('certificate_contents.updated_at asc')
                  }

  scope :has_csr, lambda {
                    not_test.joins(:certificate_contents).where do
                      (workflow_state == 'paid') &
                        (certificate_contents.signing_request != '')
                    end .order('certificate_contents.updated_at asc')
                  }
  ...
  scope :unflagged_expired_credits, lambda {
                                      unused_credits
  ...
  scope :falsely_expired, lambda {
                            unscoped.where do
                              (workflow_state == 'paid') & (is_expired == true) &
  ...
  FULL_SIGNUP_PROCESS = { label: FULL, pages: %W[Submit\ CSR Payment
                                                 Registrant Contacts #{VERIFICATION_STEP} Complete] }.freeze
  EXPRESS_SIGNUP_PROCESS = { label: EXPRESS,
                             pages: FULL_SIGNUP_PROCESS[:pages] - %w[Contacts] }.freeze
  PREPAID_FULL_SIGNUP_PROCESS = { label: PREPAID_FULL,
                                  pages: FULL_SIGNUP_PROCESS[:pages] - %w[Payment] }.freeze
  NO_CSR_SIGNUP_PROCESS = { label: PREPAID_FULL,
                            pages: PREPAID_FULL_SIGNUP_PROCESS[:pages] - %w[Submit\ CSR] }.freeze
  PREPAID_EXPRESS_SIGNUP_PROCESS = { label: PREPAID_EXPRESS,
                                     pages: EXPRESS_SIGNUP_PROCESS[:pages] - %w[Payment] }.freeze
  REPROCES_SIGNUP_W_PAYMENT = { label: FULL,
                                pages: FULL_SIGNUP_PROCESS[:pages] }.freeze
  ...
  def ucc_get_max_counts(certificate_content = nil)
    ...
    max_wildcard_count = wildcard_count if wildcard_count.present? && (wildcard_count > max_wildcard_count)
    max_nonwildcard_count = nonwildcard_count if nonwildcard_count.present? && (nonwildcard_count > max_nonwildcard_count)
  ...
  def get_reprocess_cc_domains(cc_id = nil)
    ...
    if cur_domains.any?
      cur_domains = cur_domains.map(&:signed_certificates).compact
                               .reject(&:empty?).flatten.map(&:subject_alternative_names)
    end

    cur_domains = certificate_contents.map(&:domains) if cur_domains.empty? && (renew_billing? || domains_adjust_billing?)
    cur_domains
  end
  ...
  def get_reprocess_max_nonwildcard(cc_id = nil)
    ...
    get_reprocess_cc_domains(cc_id).each do |arr|
      cur_max = arr.map { |d| d unless d.include?('*') }.compact
  ...
  def certificate
    if new_record?
      ...
      Certificate.unscoped.find_by(id: Rails.cache.fetch("#{cache_key}/certificate") do
  ...
  def used_days(options = { round: false })
    if !signed_certificates.try('empty?'.to_sym)
      sum = (Time.zone.now - (signed_certificates.min{ |a, b| a.created_at.to_i <=> b.created_at.to_i }.effective_date ||
          created_at))
  ...
  def unchain_comodo
    update_column(:external_order_number, nil) if external_order_number.present?
  ...
  def total_days(options = { round: false, duration: :order })
    if options[:duration] == :actual
      if signed_certificates.empty?
        ...
        sum = (signed_certificates.max{ |a, b| a.created_at.to_i <=> b.created_at.to_i }.expiration_date -
            signed_certificates.min{ |a, b| a.created_at.to_i <=> b.created_at.to_i }.effective_date)
        (options[:round] ? sum.round : sum) / 1.day
      end
    else
  ...
  def certificate_duration(unit = :as_is)
    Rails.cache.fetch("#{cache_key}/certificate_duration/#{unit}", expires_in: 24.hours) do
      years = if migrated_from_v2? && preferred_v2_line_items.present?
                ...
                end .scan(/\d+.+?(?:ear|ay)s?/).last
              else
                if certificate.is_ucc?
                  d = sub_order_items.includes(:product_variant_item).map(&:product_variant_item).detect(&:is_domain?).try(:description)
                  if d.present?
                    d =~ /(\d years?)/i
                    Regexp.last_match(1)
                  end
      ...
      elsif %i[comodo_api sslcom_api].include? unit
        case years.gsub(/[^\d]+/, '').to_i
        when 1
          365
        when 2
  ...
  def renewal_certificate
    if migrated_from_v2?
      ...
      Certificate.for_sale.find_by product: 'basicssl'
    else
      certificate
    end
  end
  ...
  def description
    extract = lambda {
      year = if certificate.is_ucc?
               sub_order_items.map(&:product_variant_item).detect(&:is_domain?)
             else
               sub_order_items.map(&:product_variant_item).detect(&:is_duration?)
             end
  ...
  def change_certificate(pvi)
    ...
    Order.connection.update(
      "UPDATE `orders` SET cents = #{line_items.map(&:cents).sum} WHERE id = #{order.id}"
    ) # override readonly
  ...
  def signup_process(cert = certificate)
    if skip_payment?
      ...
      if ssl_account&.has_role?('reseller')
        if cert.is_ev?
          FULL_SIGNUP_PROCESS
        else
  ...
  delegate :skip_verification?, to: :certificate
  ...
  def smime_client_process
    ...
    if registrant_types == 'iv_ov'
      ...
      !iv_validated? || certificate.is_client_pro? ? CLIENT_SMIME_IV_VALIDATE : CLIENT_SMIME_IV_VALIDATED
    else
      CLIENT_SMIME_NO_IV_OV
    end
  end
  ...
  def copy_iv_ov_validation_history(type = 'iv')
    ...
    if get_recipient && iv_exists && iv_exists.validation_histories.any?
      ...
      locked_recipient.validation_histories << new_vh if locked_recipient
  ...
  delegate :registrant, to: :certificate_content

  delegate :locked_registrant, to: :certificate_content

  delegate :csr, to: :certificate_content
  ...
  def display_subject
    ...
    names = last_signed_certificate.subject_alternative_names if last_signed_certificate.present?
    names = names.join(', ') if names.present?
    names || last_signed_certificate.try(:common_name) || csr.common_name
  end
  memoize :display_subject

  def domains
    if certificate_contents.first.domains.is_a?(Array)
  ...
  def revoke!(reason, owner = nil)
    ...
    if external_order_number.present?
      OrderNotifier.request_comodo_refund('refunds@comodo.com', external_order_number, reason).deliver
      OrderNotifier.request_comodo_refund('support@ssl.com', external_order_number, reason, 'noreply@ssl.com').deliver
      ComodoApi.revoke_ssl(certificate_order: self, refund_reason: reason)
    end
    if notes =~ /DV#(\d+)/
      OrderNotifier.request_comodo_refund('refunds@comodo.com', Regexp.last_match(1), reason).deliver
      OrderNotifier.request_comodo_refund('support@ssl.com', Regexp.last_match(1), reason, 'noreply@ssl.com').deliver
      ComodoApi.revoke_ssl(refund_reason: reason, external_order_number: Regexp.last_match(1))
    end
  ...
  def wildcard_domains
    domains.find_all{ |d| d =~ /\A\*\./ } if domains.present?
  end

  def nonwildcard_domains
    domains - nonwildcard_domains if domains.present?
  end

  # count of domains bought
  # type can be 'wildcard', 'all'
  def purchased_domains(type = 'nonwildcard')
    Rails.cache.fetch("#{cache_key}/purchased_domains/#{type&.to_s}") do
      soid = sub_order_items.find_all do |item|
        item
  ...
  def retrieve_ca_cert(email_customer = true)
    if external_order_number && certificate_content.ca.blank? &&
      ...
      if retrieve.response_code == 2
        ...
        orphaned_certificate_contents remove: true
  ...
  def self.retrieve_ca_certs(start, finish, options = {})
    Sandbox.find_by(host: options[:db]).use_database if options[:db].present?
    ...
    cos.each{ |co| CertificateOrder.unscoped.find_by(ref: co.ref).retrieve_ca_cert(true) }
    SystemAudit.create(owner: nil, target: nil,
                       notes: '',
                       action: "CertificateOrder#retrieve_ca_certs(#{start},#{finish},#{options})")
  end
  ...
  def to_api_string(options = { action: 'update' })
    ...
    when /update_dcv/
      ...
      options[:caller].blank? ?
          'curl -k -H "Accept: application/json" -H "Content-type: application/json" -X PUT -d "' +
            api_params.to_json.gsub('"', '\\"') + "\" #{domain}/certificate/#{ref}" : api_params
    when /update/
      api_params = { account_key: account_key,
                     secret_key: secret_key,
    ...
    when /show/
      api_params = { account_key: account_key,
                     secret_key: secret_key,
                     query_type: ('all_certificates' if signed_certificate.present?), show_subscriber_agreement: 'Y',
                     response_type: ('individually' if signed_certificate.present?) }
  ...
  delegate :domains_and_common_name, to: :certificate_content
  ...
  def base_api_params
    ...
    registrant_params = r.blank? ? {} :
        { organization: r.company_name,
          ...
          country: r.country }
    api_domains = {}
    if cc.domains.present?
      cc.certificate_names.includes(:domain_control_validations).find_by(domains: cc.domains.flatten + [common_name]).each do |cn|
        next unless cn
      ...
      api_domains.merge!(cc.csr.common_name.to_sym => { dcv: (last_dcv_sent ? last_dcv_sent.method_for_api : 'http_csr_hash').to_s })
    end
    api_contacts = {}
    CertificateContent::CONTACT_ROLES.each do |role|
      contact = cc.certificate_contacts.find do |certificate_contact|
        next unless certificate_contact.roles.include?(role)

        api_contact = {}
        (CertificateContent::RESELLER_FIELDS_TO_COPY + ['country']).each do |field|
          api_contact.merge! field.to_sym => certificate_contact.send(field.to_sym).to_s
        end
        api_contacts.merge! role.to_sym => api_contact
      end
    end
    [api_contacts, api_domains, cc, registrant_params]
  end
  memoize :base_api_params

  def add_renewal(ren)
    self.renewal_id = CertificateOrder.find_by(ref: ren).id if ren.present?
  end
  ...
  def bundled_cert_names(override = {})
    if ca == CA_CERTIFICATES[:SSLcomSHA2]
      if (is_open_ssl? && override[:components].blank?) || override[:is_open_ssl]
        # attach bundle
        Certificate::BUNDLES[:comodo][:sha2_sslcom_2014][:labels].select do |k, _v|
          k == if signed_certificate.try('is_ev?'.to_sym)
                 "sslcom_ev_ca_bundle#{ascending_root(override)}.txt"
        ...
        if signed_certificate.try('is_ev?'.to_sym)
          Certificate::BUNDLES[:comodo][:sha2_sslcom_2014][:contents]["sslcom_ev#{'_amazon' if is_amazon_balancer? || %w[amazon iis].include?(override[:server])}.txt"]
        elsif signed_certificate.try('is_dv?'.to_sym)
          Certificate::BUNDLES[:comodo][:sha2_sslcom_2014][:contents]["sslcom_dv#{'_amazon' if is_amazon_balancer? || %w[amazon iis].include?(override[:server])}.txt"]
        elsif signed_certificate.try('is_ov?'.to_sym)
          Certificate::BUNDLES[:comodo][:sha2_sslcom_2014][:contents]["sslcom_ov#{'_amazon' if is_amazon_balancer? || %w[amazon iis].include?(override[:server])}.txt"]
        elsif certificate.is_ev?
          Certificate::BUNDLES[:comodo][:sha2_sslcom_2014][:contents]["sslcom_ev#{'_amazon' if is_amazon_balancer? || %w[amazon iis].include?(override[:server])}.txt"]
        elsif certificate.is_essential_ssl?
          Certificate::BUNDLES[:comodo][:sha2_sslcom_2014][:contents]["sslcom_dv#{'_amazon' if is_amazon_balancer? || %w[amazon iis].include?(override[:server])}.txt"]
      ...
      if is_open_ssl? && override[:components].blank?
        # attach bundle
        Certificate::COMODO_BUNDLES.select do |k, _v|
          k == if certificate.serial =~ /256sslcom/
                 if signed_certificate.try('is_ev?'.to_sym)
                   "sslcom_ev_ca_bundle#{ascending_root(override)}.txt"
                 # elsif certificate.is_free?
                 ...
                 "trial_ssl_ca_bundle#{ascending_root(override)}.txt"
               else
                 "ssl_ca_bundle#{ascending_root(override)}.txt"
               end
        end.map{ |k, _v| k }
      else
        Certificate::COMODO_BUNDLES.select do |k, _v|
  ...
  def description_with_tier(target_order = nil)
    extract = lambda {
      ...
      tier_label = if target_order&.reseller_tier
  ...
  def request_csr_from; end

  def v2_line_items
    preferred_v2_line_items.split('|') if preferred_v2_line_items.present?
  end
  ...
  def options_for_ca(options = {})
    {}.tap do |params|
      ...
      cc.csr.tap do |csr|
        update_attribute(:ca, CA_CERTIFICATES[:SSLcomSHA2]) if ca.blank?
        if options[:new].blank? && (csr.sent_success || external_order_number)
          ...
          if [Certificate::COMODO_PRODUCT_MAPPINGS['free'], 43].include?(
            mapped_certificate.comodo_product_id
          ) # trial cert does not specify duration
          ...
          fill_csr_fields params, csr.csr_override if csr.csr_override.present?
          if false # TODO: make country override option
            override_params(params) # essentialssl
          end
          params.merge!('servers' => server_licenses.to_s || '1') if certificate.is_wildcard?
        end
  ...
  def build_comodo_dcv(last_sent = (csr.domain_control_validations.last_method || csr.last_dcv), params = {}, options = {})
    if certificate.is_ucc?
      ...
      domains_for_comodo = (options[:certificate_content] || certificate_content).all_domains
      certificate_contents.first.certificate_names
        ...
        dcv_methods_for_comodo << (last.presence || ApiCertificateCreate_v1_4::DEFAULT_DCV_METHOD_COMODO)
      end
  ...
  def reset(complete = false, ext_ca_orders = false)
    ...
    certificate_content.csr.delete if certificate_content.csr.present?

    start_over!(complete) unless %w[canceled revoked]
                                 .include?(certificate_content.workflow_state)
  end
  ...
  def self.remove_all_orphaned
    find_each{ |co| co.orphaned_certificate_contents remove: true }
  end

  # Removes the last certificate_content in the event it was a mistake
  def remove_last_certificate_content
    certificate_content.destroy if certificate_contents.count > 1
  end
  ...
  def transfer_certificate_content(certificate_content)
    site_seal.conditionally_activate! unless site_seal.conditionally_activated?
    cc = self.certificate_content
    cc.domains = certificate_content.domains
    if certificate_content.preferred_reprocessing?
      certificate_contents << certificate_content
  ...
  delegate :all_domains, to: :certificate_content

  def change_ssl_account!(acct_number)
    sa = SslAccount.find_by acct_number: acct_number
    sa.orders << order
  ...
  def certificate_contents_ref_label_switch
    certificate_contents.each do |cc|
      next if cc.ref =~ /^co-/
  ...
  def fill_csr_fields(options, obj)
    if obj.present?
      ...
      options.merge!(f.each{ |k, v| f[k] = CGI.escape(v) if v.present? })
    end
  end
  ...
  def purchase_renewal(notify)
    ...
    response = [bp, (ssl_account.cached_orders.map(&:billing_profile) - [bp]).shift].compact.each do |bp|
      ...
      RenewalAttempt.create(
        certificate_order_id: id, order_transaction_id: gateway_response.id
      )
  ...
  def clone_for_renew(_certificate_orders, order)
  ...
  def set_comodo_subca(params, options = {})
    ...
    elsif [CA_CERTIFICATES[:SSLcomSHA2]].include? ca
  ...
  def self.trial_conversions(start = 30.days.ago, finish = Date.today)
    ...
    stats = []
    count = 0
    CertificateOrder.range(start, finish).not_test.free.map{ |co| free.merge!(co.id.to_s => co.all_domains) if co.all_domains.present? }
    CertificateOrder.range(start, finish).not_test.nonfree.map{ |co| nonfree.merge!(co.id.to_s => co.all_domains) if co.all_domains.present? }
  ...
  def self.expire_credits(options = {})
    ...
    SystemAudit.create(owner: nil, target: nil,
                       notes: '',
                       action: "CertificateOrder#expire_credits(#{options})")
  end
