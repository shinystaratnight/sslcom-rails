class CertificateOrder < ApplicationRecord
  ...
  scope :search_with_csr, lambda { |term, _options = {}|
    ...
    %w[status].each do |field|
      if (query = filters[field.to_sym])
        cc_query = cc_query.where{ certificate_contents.workflow_state >> query.split(',') }
      end
    end
  ...
  PREPAID_FULL = 'prepaid_full'.freeze
  PREPAID_EXPRESS = 'prepaid_express'.freeze
  VERIFICATION_STEP = 'Perform Validation'.freeze
  CLIENT_SMIME_VALIDATE = 'client_smime_validate'.freeze
  CLIENT_SMIME_VALIDATED = 'client_smime_validated'.freeze
  CLIENT_SMIME_VALIDATED_SHORT = 'client_smime_validated_short'.freeze
  ...
  def description
    extract = lambda {
      ...
      if year.blank?
        ...
        (year.value.to_i < 365 ? "#{year.value.to_i} Days" :
  ...
  def revoke!(reason, owner = nil)
    ...
    if self.external_order_number.present?
...
=end
  ...
  def self.trial_conversions(start = 30.days.ago, finish = Time.zone.today)
