...
#  end
  ...
  def filter_by
    ...
    p = {:page => params[:page]}
    @certificate_orders = current_user.is_admin? ? CertificateOrder : current_user.ssl_account.certificate_orders
    @certificate_orders = @certificate_orders.unscoped.not_test.filter_by(params[:id]).paginate(p)
