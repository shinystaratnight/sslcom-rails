...
class ApplicationController < ActionController::Base
  ...
  def find_tier
    ...
    if current_user and current_user.ssl_account.tier_suffix
  ...
  def find_certificate_orders(options={})
    ...
    if @search = params[:search]
      ...
      (current_user.is_admin? ?
          (@ssl_account.try(:certificate_orders) || CertificateOrder).find_not_new(options) :
            current_user.ssl_account.certificate_orders.not_new(options)).order(updated_at: :desc)
    end
  end
