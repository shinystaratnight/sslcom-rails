...
class OrdersController < ApplicationController
  ...
  before_action :find_order, only: %i[show invoice update_invoice refund refund_merchant change_state
                                      edit update transfer_order update_tags]
  before_action :find_scoped_order, only: [:revoke]
  before_action :set_ssl_slug, only: :show
  before_action :set_prev_flag, only: %i[create create_free_ssl create_multi_free_ssl]
  before_action :prep_certificate_orders_instances, only: %i[create create_free_ssl]
  ...
  before_action :global_set_row_page, only: %i[index search filter_by_state visitor_trackings]
  ...
  def show_cart
    @cart = ShoppingCart.find_by(guid: params[:id]) if params[:id]
    if @cart # manually overwrite owned shopping_cart in favor of url specified
      ...
      db_cart = ShoppingCart.find_by(guid: guid)
      if current_user
  ...
  def change_quantity_in_cart
    ...
    if cookies[ShoppingCart::CART_GUID_KEY].blank?
      ...
      shopping_cart = ShoppingCart.find_by(guid: cookies[ShoppingCart::CART_GUID_KEY])
    ...
    if params[:reprocess_ucc] || params[:renew_ucc] || params[:ucc_csr_submit]
      ...
      if params[:certificate_order] && !single_cert_no_limit_order?
        ...
        unless params['prev.x'.intern].nil?
          redirect_to(buy_certificate_url(@certificate)) && return
        end

        if Settings.csr_domains_ui
          managed_domains = params[:managed_domains]
          additional_domains = ''
          if managed_domains.present?
        ...
        unless certificate_order_steps
          render(template: 'submit_csr', layout: 'application') && return
        end
      else
        # Getting Shopping Cart Info
        shopping_cart = ShoppingCart.find_by(guid: cookies[ShoppingCart::CART_GUID_KEY])
      ...
      if current_user
        if @certificate_orders && is_order_free?
          ...
          redirect_to(is_current_order_affordable? ? confirm_funds_url(:order) :
                          allocate_funds_for_order_path(id: :order)) && return
        end
      end
    end
  end
  ...
  def invoice
    @orders  = [@order]
    invoices = params[:start_date].present? && params[:end_date].present?
  ...
  def update_invoice
    ...
    update = found || Invoice.new(params[:invoice])

    no_errors = if found
                  update.update(
                    (cur_invoice || params[:invoice])
                      .keep_if { |k, _v| !%w[order_id invoice_ref].include?(k) }
                  )
                else
                  update.save
    end
  ...
  def lookup_discount
    if current_user && !current_user.is_system_admins?
      @discount = current_user.ssl_account.discounts.find_by(ref: params[:discount_code]) ||
                  Discount.viable.general.find_by(ref: params[:discount_code])
    else
      @discount = Discount.viable.general.find_by(ref: params[:discount_code])
    end
  ...
  def revoke
    if params[:revoke_all]
      ...
      co&.revoke!(params[:revoke_reason], current_user)
  ...
  def refund
    @performed = "#{params['cancel_only'] ? 'Cancelled ' : 'Refunded '} #{if params['partial']
                                                                            'partial '
                                                                        end}order"
    if @order.present?
      if params['partial'] # partial refunds or cancel line item
  ...
  def notify_ca(notes)
    @order.line_items.each do |li|
      if defined? li.sellable.external_order_number
        OrderNotifier.request_comodo_refund('refunds@ssl.com', li.sellable.external_order_number, notes).deliver
      end
      if defined?(li.sellable.external_order_number) && li.sellable.external_order_number
        OrderNotifier.request_comodo_refund('refunds@comodo.com', li.sellable.external_order_number,
                                            notes).deliver
      end
      if li.sellable.try(:notes) =~ /DV#(\d+)/
        OrderNotifier.request_comodo_refund('refunds@ssl.com', Regexp.last_match(1), notes).deliver
      end
    end
  end
    ...
    if @order.present?
      ...
      if params[:type] == 'create'
        ...
        if refund && last_refund && last_refund.successful?
          ...
          if @order.is_deposit?
            @order.get_team_to_credit.funded_account.decrement!(:cents, last_refund.amount)
          end
        else
  ...
  def index
    ...
    if is_sandbox? && @search.include?('is_test:true').blank?
      @search << ' is_test:true'
    end
    @unpaginated =
      if @search.present?
  ...
  def filter_by_state
    ...
    @unpaginated =
      if current_user.is_admin?
        ...
        current_user.ssl_account.orders.unscoped do
  ...
  def show
    ...
    @taggable = @order
    get_team_tags
    if /Deposit|Funded Account Withdrawal/i.match?(@order.description)
      @deposit = @order
    elsif @order.line_items.count == 1
      @certificate_order = @order.cached_certificate_orders.uniq.last
    else
      certificates = []
      @certificate_orders = @order.cached_certificate_orders.uniq.map do |co|
  ...
  rescue Payment::AuthorizationError => e
  ...
  def create_free_ssl
    @order = Order.new(params[:order])
    @user = User.new(params[:user]) unless current_user
    respond_to do |format|
      if @certificate_orders
        @order.add_certificate_orders(@certificate_orders)
      else
      ...
      if (@order.cents == 0) && !@order.line_items.empty? && (@user ? @user.valid? : true)
        save_user unless current_user
        current_user.ssl_account.orders << @order
        record_order_visit(@order)
        @order.give_away!
  ...
  def create_multi_free_ssl
    ...
    respond_to do |format|
      if @user && !@user.valid?
        format.html { render action: 'new' }
      elsif (@order.cents == 0) && !@order.line_items.empty? && (@user || current_user)
        save_user unless current_user
        current_user.ssl_account.orders << @order
        record_order_visit(@order)
        @order.give_away!
  ...
  def single_cert_no_limit_order?
    ssl_account&.no_limit && request.referer && request.referer.include?('certificates')
  end
  ...
  def smime_client_enrollment_hybrid_payment
    if current_user && order_reqs_valid? && !@too_many_declines && purchase_successful?
      ...
      if @order.invoiced?
        ...
        if @funded_amount > 0
          withdraw_funded_account((@funded_amount * 100).to_i)
        end
      end
      record_order_visit(@order)
  ...
  def reprocess_ucc_order_free(_params)
  ...
  def refund_partial_amount(params)
    ...
    all_refunded  = @order.line_items.count{ |li| li.sellable.try('refunded?') } == @order.line_items.count
  ...
  def refund_partial_cancel(_params)
  ...
  def cancel_entire_order
    ...
    @target.update(invoice_id: nil) if @target.canceled? && @target.invoice
  end
  ...
  def checkout_shopping_cart_content(content, cart)
    ...
    cart.each do |cookie|
      same = content.detect do |cont|
        cont[ShoppingCart::LICENSES] == cookie[ShoppingCart::LICENSES] &&
          cont[ShoppingCart::DOMAINS].present? && cookie[ShoppingCart::DOMAINS].present? &&
      ...
      same[ShoppingCart::QUANTITY] = cookie[ShoppingCart::QUANTITY].to_i if same.present? &&
                                                                            same[ShoppingCart::QUANTITY].to_i != cookie[ShoppingCart::QUANTITY].to_i

      new_contents << (same.presence || cookie)
    end
    new_contents = '' if new_contents.empty?
