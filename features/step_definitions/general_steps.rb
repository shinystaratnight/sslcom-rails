...
When /\A(?:he|she|I) (uncheck|check) the checkbox with ['"]([^'"]*)['"](?:\s)?['"]([^'"]*)['"]\z/ do |status, text, attribute|
  @browser.checkbox(attribute.intern, Regexp.new(text)).checked = status == 'check'
end
...
Then /\A(?:he|she|I) should see a link pointing to ['"]([^'"]*)['"]\z/ do |path|
  @browser.link(:href, path.to_s).text.downcase.should include(text.downcase)
end
...
ParameterType(
  name: 'username',
  regexp: /[A-Z][a-z]+/,
  transformer: -> (name) { @current_user || User.find_by(login: name) }
)
...
Then /\A(\S+) should be denied\z/ do |_user|
